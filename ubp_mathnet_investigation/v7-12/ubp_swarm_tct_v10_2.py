"""
================================================================================
UBP SWARM ORCHESTRATOR — TCT EDITION v10.2 "OMEGA FINAL"
================================================================================
Author: UBP Research Cortex v5.0
Date: 26 April 2026

FINAL HARDENING:
- Fixed "Dual" to "complex" casting error in nested derivatives.
- Fixed "y is not defined" error by providing a multi-variable eval environment.
- Implemented deep-unwrapping for all ALU outputs.
================================================================================
"""
...
log = logging.getLogger("UBP_TCT_v10_2")
...
class SovereignPhysicist:
    ...
    def prove(self, parsed: ParsedDirective) -> dict:
        ...
        # Provide x and y in the environment to prevent "name not defined" errors
        ...
        try:
            if parsed.op == "second_derivative":
                # Fix: Ensure the inner function returns a raw float/complex for the outer derivative
                ...
                # Fix: Replace the target variable with x and provide y=1.0
...
class UBPSwarmTCTv10_2:
    ...
    def run(self, directives: List[str], output_md: str = "v10_2_final_report.md"):
        steps = [self.run_directive(d) for d in directives]
        md = "# UBP TCT v10.2 — Omega Final Report\n\n"
        for s in steps:
            md += f"## {s['title']}\n\n"
...
if __name__ == "__main__":
    SUITE = [
        ...
        "Compute the partial derivative of x**2 + y**2 w.r.t. x",
    ...
    swarm = UBPSwarmTCTv10_2(training_steps=75000)
    swarm.run(SUITE)