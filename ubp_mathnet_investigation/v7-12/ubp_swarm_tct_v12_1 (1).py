...
"""
================================================================================
UBP SWARM ORCHESTRATOR — TCT EDITION v12.1 "OMNI-SYNTHESIS"
================================================================================
Author: UBP Research Cortex v5.0
Date: 26 April 2026

FIXES:
- Restored the full parse_directive engine (accidentally hardcoded to "concept" in v12.0).
- Merged v11.4 NLP-Math Bridge and Absolute Zero Calculus with v12.0 Deep Synthesis.
================================================================================
"""
...
log = logging.getLogger("UBP_TCT_v12_1")
...
def parse_directive(text: str) -> ParsedDirective:
    ...
    if m_add: return ParsedDirective(text, "arithmetic", expr=f"{m_add.group(2)} + {m_add.group(1)}", nums=nums, concepts=valid_concepts)
    m_sub = re.search(r"subtract\s+(-?\d+(?:\.\d+)?)\s+from\s+(-?\d+(?:\.\d+)?)", text, re.I)
    if m_sub: return ParsedDirective(text, "arithmetic", expr=f"{m_sub.group(2)} - {m_sub.group(1)}", nums=nums, concepts=valid_concepts)
    m_mul = re.search(r"multiply\s+(-?\d+(?:\.\d+)?)\s+by\s+(-?\d+(?:\.\d+)?)", text, re.I)
    if m_mul: return ParsedDirective(text, "arithmetic", expr=f"{m_mul.group(1)} * {m_mul.group(2)}", nums=nums, concepts=valid_concepts)
    m_div = re.search(r"divide\s+(-?\d+(?:\.\d+)?)\s+by\s+(-?\d+(?:\.\d+)?)", text, re.I)
    if m_div: return ParsedDirective(text, "arithmetic", expr=f"{m_div.group(1)} / {m_div.group(2)}", nums=nums, concepts=valid_concepts)
...
class LanguageScribe:
    ...
    def write(self, directive: str, concepts: List[str], context: str = "", answer: Optional[float] = None, nrci: float = 0.0, max_words: int = 60, freelance: List[str] = []) -> dict:
        freelance_context = " ".join(freelance)
        base_query = f"{directive} {' '.join(concepts)} {freelance_context} geometric resonance nrci={nrci:.4f}"
        if answer is not None:
            base_query += f" computed value {answer:.6g}"

        text = self.moe.research(base_query, max_words=max_words)
        text = re.sub(r"entropy resonance substrate is the substrate", "", text, flags=re.I).strip()
        
        if len(text.split()) < 12:
            text = (f"The directive '{directive[:30]}...' manifests as a geometric tension in the 24-bit manifold "
...
class Director:
    def synthesize(self, steps: List[dict]) -> str:
        md = "# UBP TCT v12.1 — Omni-Synthesis Report\n\n"
        for s in steps:
            sov = s["sovereign"]
            lang = s["language"]
            crit = s["critic"]
...
class UBPSwarmTCTv12_1:
    ...
    def run_directive(self, directive: str) -> dict:
        ...
        lang_col = self.scribe.write(directive, parsed.concepts, context=directive, answer=sov["answer"], nrci=sov["nrci"], freelance=freelance_insights)
        
        report = self.critic.audit(sov, lang_col)
        if report["severity"] != "accepted" and not sov["error"]:
            lang_col = self.scribe.write(directive, parsed.concepts + ["entropy", "coherence", "deep thermodynamic insight"], context=directive, answer=sov["answer"], nrci=sov["nrci"], max_words=80, freelance=freelance_insights)
            report = self.critic.audit(sov, lang_col)
            
        return {"title": directive, "sovereign": sov, "language": lang_col, "critic": report, "freelance": freelance_insights}

    def run(self, directives: List[str], output_md: str = "v12_1_omni_synthesis_report.md"):
