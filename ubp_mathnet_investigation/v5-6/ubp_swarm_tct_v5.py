"""
================================================================================
UBP SWARM ORCHESTRATOR — TCT EDITION v5.1 "THE BRAIN-AWARE OFFICE"
================================================================================
Author: UBP Research Cortex v5.0
Date: 21 April 2026

INTEGRATED ARCHITECTURE (STANDALONE):
- Tier 0-3: Brain-Aware Trinity (Math, Python, Language).
- Tier 4: TCT Auditor (Phase-Lock Verification).
- Tier 5: Critic Agent (Semantic Reflection & Scientific Grounding).
- Tier 6: Director (Synthesis & Office Meeting Notes).
- Tier 7: TGIC Relational Synthesis (Manifold Gravity).
- Tier 8: Ontological Harvester (Recursive Learning).
================================================================================
"""
...
logger = logging.getLogger("UBP_OFFICE_v5")
...
try:
    ...
    ENGINES_OK = False
    logger.warning(f"Engine import error: {e}. Running in degraded mode.")
...
class LanguageScribeEngine:
    ...
    def write(self, concept: str, math_col: MathColumn, py_col: PythonColumn, platform: str, revised_prompt: str = None) -> LanguageColumn:
        target = LEECH_PLATFORMS[platform]["target_nrci"]
        
        if revised_prompt and self.moe:
            logger.info(f"  [Refining] {concept} using Brain-Aware prompt...")
            try:
                para = self.moe.research(revised_prompt, max_words=40)
                if len(para.split()) > 10:
                    header = f"The concept of **{concept}** ({platform}) is tuned to NRCI {math_col.nrci:.4f}. "
                    para = header + para
                else:
                    raise ValueError("MoE output too short")
            except:
                para = self._default_para(concept, math_col, py_col, platform, target)
        else:
            para = self._default_para(concept, math_col, py_col, platform, target)

        if not para.endswith('.'): para += '.'
        resonance = 0.85 - (abs(math_col.nrci - target) * 2)
        return LanguageColumn(math_col.step_id, concept, para, len(para.split()), resonance, float(math_col.nrci), True)

    def _default_para(self, concept, math_col, py_col, platform, target):
        return (f"The concept of **{concept}** is geometrically anchored to the **{platform}** platform "
...
class CriticAgent:
    """The 'Gut' — now Brain-Aware, queries the semantic engine to check grounding."""
    def __init__(self, semantic_engine):
        self.semantic = semantic_engine

    def reflect(self, step: TCTStep) -> Dict[str, Any]:
        critique = {"coherence": 1.0, "depth_issues": [], "suggestions": [], "revised_prompt": ""}
        ...
        if len(step.language.paragraph.split()) < 45:
            critique["depth_issues"].append("Insufficient narrative density")
            critique["suggestions"].append("Expand on entropy-driven chelate stabilization")
            critique["coherence"] -= 0.3

        critique["revised_prompt"] = (
            f"Reflect deeply on {step.step_title} in {step.platform} platform. "
            f"The geometric manifold {step.math.voxel_path} encodes stability via "
            f"entropy gains and cooperative hydrogen bonding. "
            f"Generate a dense paragraph linking UBP geometry to real thermodynamic stabilization."
        )
        return critique

class Director:
    """The Boss: Synthesizes the final report with meta-cognitive notes."""
    def __init__(self, semantic_engine):
        self.semantic = semantic_engine

    def synthesize(self, steps: List[TCTStep], tgic: Dict, critiques: List[Dict], directive: str) -> str:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        md = f"# TCT v5.1 Office Report — {directive}\n\n"
        ...
        for i, step in enumerate(steps):
            ...
            md += f"- **Critic Reflection:** {', '.join(critiques[i]['suggestions']) or 'Strong coherence.'}\n\n"
        ...
        md += "The Critic's intervention ensured that geometric complexity was translated into scientific insight.\n"
        return md

class TGICRelationalAuditor:
    """Tier 7: Evaluates the 3-6-9 structural stability of the entire document manifold."""
    def __init__(self):
        self.tgic = TGICExactEngine() if ENGINES_OK else None

    def evaluate_manifold(self, steps: List[TCTStep]) -> Dict[str, Any]:
        if not self.tgic or not steps:
            return {"status": "TGIC Engine Unavailable"}
        ...
        node_reports = []
        for step in steps:
            vec = step.math.vector
            coord = step.step_id
            node_energy = self.tgic.get_node_energy(coord, vec, S)
            relational_pull = self.tgic.get_relational_pull(coord, vec, S)
            structural_stability = self.tgic.interactions.calculate_total_stability(vec, manifold_vecs)
            
            node_reports.append({
                "concept": step.step_title,
                "platform": step.platform,
                "node_energy": float(node_energy),
                "relational_pull": float(relational_pull),
                "structural_stability": float(structural_stability)
            })
            
        return {
            "status": "SUCCESS",
            "total_manifold_energy": float(total_energy),
            "nodes": node_reports
        }

class OntologicalHarvester:
    """Tier 8: Commits stable concepts to the Learning Sidecar."""
    @staticmethod
    def harvest(steps: List[TCTStep], directive_hash: str, manifold_energy: float):
        if manifold_energy > 250.0:
            logger.warning(f"Manifold Energy ({manifold_energy:.2f}) too high. Aborting harvest.")
            return 0
        ...
        for step in steps:
            if step.accepted:
                ...
                entry = [
                    uid,
                    f"[Learned: {step.step_title}], {step.language.paragraph[:150]}...",
                    ["LEARNED", "TCT_SWARM", datetime.now().strftime("%Y-%m-%d")],
                    step.math.vector,
                    step.math.nrci
                ]
        ...
        if new_count > 0:
            ...
            logger.info(f"🚀 HARVEST COMPLETE: {new_count} concepts added to {LEARNING_FILE}")
        return new_count
...
class UBPSwarmTCTv5:
    def __init__(self, directive: str, num_steps: int = 3):
        ...
        # 1. Initialize Semantic Engine & MoE
        ...
        if self.semantic:
            ...
            if os.path.exists(LEARNING_FILE):
                with open(LEARNING_FILE, 'r') as f:
                    self._hydrate_learned(json.load(f))
    ...
    def run(self):
        logger.info(f"Starting UBP TCT OFFICE v5.1 | Directive: {self.directive}")
        ...
        # B. Reflection Round
        logger.info("Entering Reflection Round...")
        critiques = []
        final_steps = []
        for step in initial_steps:
            crit = self.critic.reflect(step)
            critiques.append(crit)
            
            if crit["coherence"] < 0.8:
                # Re-generate with Critic's prompt
                refined_lang = self.lang_engine.write(
                    step.step_title, step.math, step.python, step.platform, crit["revised_prompt"]
                )
                step.language = refined_lang
                logger.info(f"  [Refined] {step.step_title} depth increased.")
            
            final_steps.append(step)

        # C. Relational Synthesis & Harvesting
        ...
        # D. Final Synthesis
        report_md = self.director.synthesize(final_steps, tgic_report, critiques, self.directive)
        
        with open("tct_v5_office_report.md", "w", encoding="utf-8") as f:
            f.write(report_md)
