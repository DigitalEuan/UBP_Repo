...
def toggle_difference(b_i: Union[int, OffBit], b_j: Union[int, OffBit]) -> Union[int, OffBit]:
    """
    Perform DIFFERENCE toggle operation.
    
    Axiom: |b_i - b_j|
    Purpose: Absolute difference between two OffBits
...
def toggle_xor(b_i: Union[int, OffBit], b_j: Union[int, OffBit]) -> Union[int, OffBit]:
    """
    Perform TRUE BINARY XOR toggle operation.
    
    Axiom: b_i ^ b_j (bitwise XOR)
    Purpose: Binary exclusive OR at the bit level
    ...
    Returns:
        Result of binary XOR operation
    """
    # Extract values if OffBits
    val_i = b_i.value if isinstance(b_i, OffBit) else b_i
    val_j = b_j.value if isinstance(b_j, OffBit) else b_j
...
toggle_xor_legacy = toggle_difference
