...
try:
    from hardware_profiles import HardwareProfileManager, HardwareProfile
except ImportError:
    # If hardware_profiles not available, use Any for type hints
    from typing import Any as HardwareProfile
    HardwareProfileManager = None
...
class ConstantConfig:
    ...
    UBP_FREQUENCY_WEIGHTS: Dict[float, float] = field(default_factory=lambda: RawUBPConstants.get_frequency_weights().copy())
    UBP_TOGGLE_PROBABILITIES: Dict[str, float] = field(default_factory=lambda: RawUBPConstants.get_toggle_probabilities().copy())
