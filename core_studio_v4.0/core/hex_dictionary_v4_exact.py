...
class HexDictionaryV4Exact:
    ...
    def mint_rational_vector(self, ubp_id: str) -> List[int]:
        """Assigns a vector based on the Rational ULAP Standard."""
        uid = ubp_id.upper()
        # Domain Mapping
        domains = {"ELEM":0, "CHEM":0, "PHYS":1, "MECH":1, "QM":1, "BIO":2, "CELL":2, "CODE":3, "ALGO":3, "CONST":4, "NUM":4, "LAW":5}
        dom_val = 7 # Default Meaning
        for prefix, val in domains.items():
            if prefix in uid: dom_val = val; break
        
        # Extract Sequence (e.g. 069 from ELEM_Tm_069)
        nums = re.findall(r'\d+', uid)
        seq = int(nums[-1]) if nums else 0
        
        # Construct 12-bit message: [Domain(3) | Scale(3) | Seq(5) | Flux(1)]
        msg = [(dom_val >> i) & 1 for i in range(2, -1, -1)] # Domain
        msg += [0, 0, 0] # Scale
        msg += [(seq >> i) & 1 for i in range(4, -1, -1)] # Seq
        msg += [0] # Flux
