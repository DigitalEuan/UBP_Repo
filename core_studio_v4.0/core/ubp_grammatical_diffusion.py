"""
Grammatical Diffusion Reasoner (GDR) — v3.3 (Optimized Recursive Edition)
========================================================================
Implements a search through the zoned lattice, guided by the Grammar FSM,
using A* search with stateful attraction potentials and integrated CRG semantic links.
"""
    ...
    def reason(self, start_lemma: str, target_lemma: str, max_depth: int = 7) -> ReasonerTrace:
        ...
        while open_set:
            ...
            if lemma == target_lemma and current_fsm.is_accepting():
                return ReasonerTrace(path, True, path[-1].nrci, Fraction(sum(s.tax for s in path)))

            # 1. Semantic neighbors (from CRG) - High priority, low cost (v3.3)
            if self.crg and lemma in self.crg.out:
                for edge in self.crg.out[lemma]:
                    next_word = self._get_word(edge.dst)
                    if not next_word: continue
                    z_next = dominant_zone(next_word.vector)
                    if current_fsm.peek(z_next):
                        temp_fsm = GrammarFSM(); temp_fsm.state = state_name; temp_fsm.step(edge.dst, z_next)
                        step_tax = Fraction(1, 10)
                        new_g = g + float(step_tax) + 0.5
                        h = (BLA.hamming_distance(next_word.vector, target_word.vector) / 24.0)
                        if (edge.dst, temp_fsm.state) not in visited:
                            heapq.heappush(open_set, (new_g + h, new_g, edge.dst, temp_fsm.state, path + [self._make_step(edge.dst, next_word, z_next, step_tax)]))

            # 2. Geometric neighbors (Standard A*)
            ...
            for next_lemma, next_word in neighbors:
                ...
                if current_fsm.peek(z_next):
                    ...
                    if (next_lemma, temp_fsm.state) not in visited:
                        heapq.heappush(open_set, (new_g + h, new_g, next_lemma, temp_fsm.state, path + [self._make_step(self._get_lemma(next_word), next_word, z_next, step_tax)]))

            # 3. Compositional neighbors
                        ...
                        for subjects in [[path[0].word, target_lemma], target_lemma]:
                            ...
                            if shifted:
                                ...
                                if temp_fsm.peek(z_sh):
                                    ...
                                    h = (BLA.hamming_distance(shifted.vector, target_word.vector)/24.0)
                                    if (self._get_lemma(shifted), temp_fsm.state) not in visited:
                                        heapq.heappush(open_set, (new_g+h, new_g, self._get_lemma(shifted), temp_fsm.state, path + [self._make_step(self._get_lemma(op_word), op_word, z_op, op_tax), self._make_step(self._get_lemma(shifted), shifted, z_sh, sh_tax)]))
