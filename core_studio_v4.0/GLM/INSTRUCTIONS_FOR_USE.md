# GLM v3.7.1 — Instructions for Use

This document covers everything needed to run the refined Geometric Language
Machine. It is intentionally minimal: prerequisites, setup, three commands,
the API, and the confidence tags.
...
(Or edit the `UBP_CORE_PATH` constant at the top of `glm_v37_unified.py`
line ~51 to point at your `/core/` directory — then you don't need the env
var.)
...
# v3.7.1 NEW: chat with effort — iteratively matures if not crystallized
...
| `[forming]` | **v3.7.1**: idea has evidence but hasn't crystallized yet — names the topic nouns being gathered |
...
These are imported (directly or transitively) by `glm_v37_unified.py`.
...
- `ubp_critpt_sovereign_v3.py` — `GLMRulesEngine`
...
runtime to function. They may be useful for future development but are dead
weight for the live system:

- `glm_strict_vocabulary.json` (11.9MB) — a snapshot; runtime rebuilds vocab fresh
- `hash_memory_kb.json` (253KB) — never opened by glm_v37
...
- `auto_trigger.py` — UBP-App-specific, never imported by glm_v37
...
- **Boot time**: ~3 seconds (2,338-word vocab + 127-edge CRG + 6 auto + 55 numbers)
...
`glm_v37_unified.py` sets to `UBP_CORE_PATH` on import). Copy it there:
```bash
cp UBP_Repo/core_studio_v4.0/system_kb/ubp_system_kb.json \
   UBP_Repo/core_studio_v4.0/core/ubp_system_kb.json
```
...
## v3.7.1 Refinement Notes

This version includes two targeted refinements over v3.7:

1. **User-friendly "still forming" fallback** — when a query produces
   evidence but the idea hasn't crystallized, the response now says
   `[forming] gathering evidence around <topic nouns>` instead of raw
   internal diagnostic state (`[idea: forming | coh=0.50...]`). The raw
   diagnostic only shows when the idea has both topic nouns AND a CRG
   backbone (i.e., it's actually forming, not just seeded).

2. **`chat_with_effort(query, max_ticks=5)`** — a new method that calls
   `chat()` then, if the idea hasn't crystallized, calls `mature(1)` and
   re-composes the response, up to `max_ticks` times. Useful for queries
   that need more reasoning steps. Does NOT change default `chat()` behavior.

**Verified**: 12/12 self-tests still pass. Gold-set accuracy improved from
6/28 (21.4%) to 9/28 (32.1%) — +3 cases, 0 regressions.
