...
class TGICExactEngine:
    ...
    def gamma_sphere(self, off: OffBit) -> bool:
        ...
        # Convert 0/1 to -1/1 for Leech check
        coords = tuple((2 * b - 1) for b in off.v)
        # Note: v4.3 LeechPointScaled expects integer coords.
        # We check if it maps to a valid shell.
        # For speed, we just check if it's a valid Golay Codeword + Noise
        # Actually, let's use the Syndrome check.
