...
"""
UBP-Refined Benchmark
Run same queries through refined UBP system (NRCI threshold 0.80)
"""
...
def main():
    print("=" * 80)
    print("UBP-REFINED BENCHMARK (Improved System)")
    print("=" * 80)
    print("NRCI threshold: 0.80 (refined)")
    print("Testing 8 queries with full 7-layer UBP validation + improvements")
    print()
    
    # Create UBP pipeline with refined config
    config = ImprovedPipelineConfig()  # Already has improved thresholds
    
    # Use improved pipeline
    base_pipeline = ImprovedUBPPipeline(config=config)
    ...
    output = {
        'model': 'gpt-4.1-nano',
        'system': 'UBP-refined (improved, NRCI=0.80)',
        'test_count': len(results),
        'results': results
    }
    
    with open('ubp_refined_results.json', 'w') as f:
        json.dump(output, f, indent=2)
    
    print(f"Results saved to: ubp_refined_results.json")
